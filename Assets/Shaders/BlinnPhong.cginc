...
fixed4 MyFragmentProgram(v2f i) : SV_Target
{
    ...
    float attenuation = 1.0 / (1 + pow(distance(_WorldSpaceLightPos0.xyz, i.worldPos), 2));
    ...
    float3 V = normalize(_WorldSpaceCameraPos.xyz - i.worldPos); // View direction
    float3 H = normalize(L + V); // Halfway vector
    float specularShade = pow(max(dot(N, H), 0.0), _Shininess);
    specular = specularShade * _SpecularColor * _LightColor0 * attenuation;

    // Ambient component
    float3 ambient = float3(0, 0, 0);
    ambient = UNITY_LIGHTMODEL_AMBIENT.rgb * _LightColor0;
